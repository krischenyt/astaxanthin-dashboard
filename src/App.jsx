...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 8, padding: "10px 14px", fontSize: 12, minWidth: 160 }}>
      {fullDate && (
        <div style={{ color: "#f8fafc", marginBottom: 8, fontWeight: 700, fontSize: 13, borderBottom: "1px solid #334155", paddingBottom: 6 }}>
          {fullDate}
        </div>
      )}
      {validPayload.map(p => (
        <div key={p.dataKey} style={{ color: p.color, marginBottom: 3, display: "flex", justifyContent: "space-between", gap: 16 }}>
          <span>{SKUs.find(s => s.key === p.dataKey)?.label || p.name}</span>
          <b>{(p.value || 0).toLocaleString()}</b>
        </div>
      ))}
    </div>
  );
          ...
          <div style={s.chart}>
            ...
            <ResponsiveContainer width="100%" height={230}>
              <LineChart data={combinedDaily} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
                ...
                <Tooltip content={<ChartTooltip />} labelFormatter={l => { const row = combinedDaily.find(r => r.label === l); return row?.date || l; }} />
